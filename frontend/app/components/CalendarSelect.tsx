...
export default function CalendarSelect({ 
  ...
  return (
    <div className={`relative ${className}`} dir="rtl" ref={dropdownRef}>
      ...
      {isOpen && (
        <div
          className="absolute w-max min-w-full bg-white border border-[#D0D0D4] rounded z-[9999] overflow-hidden p-1 top-full mt-1"
          style={{ 
            maxHeight: '200px', 
            overflowY: 'auto',
          ...
          <div dir="rtl" className="flex flex-col gap-0.5">
            {options.map((option) => (
              <button
                ...
                className={`
                  w-full px-2 py-1.5 text-right text-sm rounded-sm transition-colors whitespace-nowrap
                  ${value === option.value 
