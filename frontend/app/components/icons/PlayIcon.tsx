...
export default function PlayIcon({ className = "w-12 h-12", color = "white" }: PlayIconProps) {
  return (
    <span className={`relative inline-block ${className}`}>
      {/*
        Frosted circle. Rendered as a real HTML element (not an SVG
        <foreignObject>) and kept BEHIND the arrow. On mobile (iOS Safari /
        Chrome) a foreignObject with backdrop-filter creates a compositing
        layer that paints ABOVE later SVG siblings, so the blur ended up over
        the arrow — looking like a bug. As a plain positioned element behind
        the arrow SVG, the paint order is reliable everywhere.
      */}
      <span
